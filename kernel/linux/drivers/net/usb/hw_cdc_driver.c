...
/////////////////////////////////////////////////////////////////////////////////////////////////
#define DRIVER_VERSION "v2.11.00.00"
...
#define BINTERFACESUBCLASS_HW 0x03 //DTS2013061403406 added by l81005329 20130614
///////////////////////////////////////////////////////////////////////////////////////////
...
#endif
bool deviceisBalong = false;  //DTS2013061403406 added by l81005329 20130614
...
static int hw_change_mtu (struct net_device *net, int new_mtu)
{
	...
	printk("change mtu :%d, urb_size:%u",new_mtu,(u32)dev->rx_urb_size);
...
static void rx_submit (struct hw_cdc_net *dev, struct urb *urb, gfp_t flags)
{
    ...
    //if ((skb = alloc_skb (size + NET_IP_ALIGN, flags)) == NULL) {//matt mask for BRCM usb host ehci issue;
   ...
   // skb_reserve (skb, NET_IP_ALIGN);	//matt mask for BRCM usb host ehci issue;
...
void hw_disconnect (struct usb_interface *intf)
{
    ...
    /*ͬ\B2\BDȡ\CF\FB\BF\C9\C4\DCע\B2\E1\B5\C4\D1ӳٹ\A4\D7\F7\BD\F8\B3̣\AC\C8\E7\B9\FB\B8ù\A4\D7\F7\BD\F8\B3\CC\D2Ѿ\AD\D4\DAִ\D0\D0\C1ˣ\AC
    \D4\F2\D4\DA\D5\E2\C0\D0\E8Ҫ\B5ȴ\FD\B8ù\A4\D7\F7\BD\F8\B3\CCִ\D0\D0\CD\EA\B3\C9֮\BA\F3\A3\ACdiscconect\B2Ż\E1\BC\CC\D0\F8\CD\F9\CF\C2ִ\D0С\A3*/
...
static const struct net_device_ops hw_netdev_ops = {
    ...
    .ndo_get_stats = hw_get_stats, //\D3\C3\D3\DA\C1\F7\C1\BFͳ\BC\C6
};
#endif
...
//    DECLARE_MAC_BUF(mac);
    deviceisBalong = false; //DTS2013061403406 added by l81005329 20130614
  ...
  */
    if (!driver->supports_autosuspend) {
         ...
         //pm_runtime_enable(&udev->dev); matt for compile issue
    }
    ...
    if (!net) {
        //dbg ("can't kmalloc dev");//DTS2013061702489 delete by l81005329 20130617
        goto out;
    }
...
/* linux kernel > 2.6.37: PowerManager needs disable_depth ==0 */
        ...
        {
         // dev->intf->dev.power.disable_depth = 0;matt for compile issue
        }
#endif
/* Add for DTS2011050903736 lxz 20110520 end*/
    ...
    if (dev->is_ncm){
        ...
        dev->rx_urb_size = 4096;//dev->hard_mtu;//matt mask for BRCM usb host ehci issue;
    ...
    if (0 < hw_send_tlp_download_request(udev)){
        devdbg(dev, "%s: The tlp is activated", __FUNCTION__);
		 printk("%s: The tlp is activated", __FUNCTION__);
    ...
    netif_carrier_off(net);
     ...
     if(!deviceisBalong) //DTS2013061403406 added by l81005329 20130614
    {
        dev->qmi_sync = 0;
...
static int hw_cdc_bind(struct hw_cdc_net *dev, struct usb_interface *intf)
{
    ...
    while (len > 3) {
        ...
        case USB_CDC_UNION_TYPE:
		    ...
		    //DTS2013061403406 added by l81005329 20130614
...
next_desc:
    ...
    /*support the data and contorl interface*/
    if (((NULL != info->u)&&(info->u->bMasterInterface0 != info->u->bSlaveInterface0)))
    {
	 ...
	 || ((NULL != info->u)&&(info->u->bMasterInterface0 == info->u->bSlaveInterface0)))
    {
     ...
     && BINTERFACESUBCLASS != intf->cur_altsetting->desc.bInterfaceSubClass
	 && BINTERFACESUBCLASS_HW != intf->cur_altsetting->desc.bInterfaceSubClass) {
	     printk(KERN_ERR"lxz device is Qualcomm device bcdDevice=%x,InterfaceSubClass=%x\n",
...
static const struct usb_device_id    hw_products [] = {
    /*ɾ\B3\FD\B6\D4PRODUCT ID\B5ıȽϣ\ACĬ\C8\CF\C4ܹ\BB֧\B3\D6\CB\F9\D3\D0HUAWEI_ETHER_INTERFACE \BDӿ\DA\C0\E0\D0͵\C4NDIS\C9豸*/
    /* delete by lKF36757 2011/12/26,prevent hilink load hw_cdc_driver.ko*/
