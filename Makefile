...
$(error "PCTYPE=15 does not support 3385 processor")
		endif
	endif
...
$(error "There is no DSP support for 3383 with PCTYPE=20 DECT=int")
				...
				ifeq ($(strip $(DECTMODE)),ext)
# 3383 DECT=ext
					PCIMAGE_VALID_NAMES := g711_faxr all_codecs_wb bv16_ilbc_g728_g729e_amrwb_faxr_wb bv16_ilbc_g728_g729e_faxr bv32_g722_amrwb_faxr_wb comcast_wb g722_faxr_wb all_codecs
...
$(error "There is no DSP support for 3385 with PCTYPE=20 DECT=ext or DECT=0")
			endif
		endif
	endif
...
ifeq ($(strip $(DECTMODE)),int)
	# set option to build dectd under target 'userspace'
	export BUILD_DECTD=y
...
# TP1 (Less Filling): 
# Fake load address to take into account bootloader adding 128M
# Which it does in the Zephyr case
KERNEL_LOAD_ADDRESS=0x7e000000
endif
else
# Zephyr
...
ifeq ($(wildcard $(KERNEL_DIR)/vmlinux),)
	...
	echo "#define CONFIG_BCM_PCTYPE \"$(CONFIG_BCM_PCTYPE)\"" >> $(KERNEL_DIR)/include/linux/linux_build_info.h; \
	echo "#define CONFIG_BCM_CODECTYPE \"$(CONFIG_BCM_CODECTYPE)\"" >> $(KERNEL_DIR)/include/linux/linux_build_info.h; \
	echo "#define CONFIG_BCM_PCIMAGE \"$(CONFIG_BCM_PCIMAGE)\"" >> $(KERNEL_DIR)/include/linux/linux_build_info.h; \
	echo "#define CONFIG_BCM_NO_DSP \"$(CONFIG_BCM_NO_DSP)\"" >> $(KERNEL_DIR)/include/linux/linux_build_info.h; \
	$(MAKE) oldconfig; $(MAKE)
else
	echo "#define FS_KERNEL_IMAGE_NAME \"$(FS_KERNEL_IMAGE_NAME)_kernel\"" > $(KERNEL_DIR)/include/linux/linux_build_info.h; \
	echo "#define MAKEFLAGS \"$(MAKEFLAGS)\"" >> $(KERNEL_DIR)/include/linux/linux_build_info.h; \
...
initrd_buildimage: vmlinux
	cd $(PROFILE_DIR); \
	$(HOSTTOOLS_DIR)/ProgramStore2 -f vmlinux.bin -o $(INITRD_IMAGE_NAME)_initrd_sto.bin -v 003.000 -a $(KERNEL_LOAD_ADDRESS) -c 4 -s 0x$(BRCM_CHIP);
