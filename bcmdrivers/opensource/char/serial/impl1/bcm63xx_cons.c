...
/* hack until voice config options are properly implemented */
#define CONFIG_BCM_SERIAL_CONSOLE_ASYNC 1
...
/* CONFIG_MIPS_BRCM_UART_BASE takes precedence over all others */
#if defined(CONFIG_MIPS_BRCM_UART_BASE)
#undef UART_BASE
#define UART_BASE CONFIG_MIPS_BRCM_UART_BASE
#endif
...
static void bcm63xx_tx_chars(struct uart_port *port)
{
	...
	struct circ_buf *circ = (struct circ_buf *)(port->private_data);
	...
	if (circ) {
		while ((!uart_circ_empty(circ)) &&
			   ((UART_REG(port))->txf_levl < port->fifosize)) {
			(UART_REG(port))->Data = circ->buf[circ->tail];
...
#ifdef CONFIG_BCM_SERIAL_CONSOLE_ASYNC

static void
bcm63xx_async_console_write(struct console *co, const char *s, unsigned int count)
{
	unsigned i;
	struct uart_port *port = &bcm63xx_ports[co->index];
	struct circ_buf *circ = (struct circ_buf *)(port->private_data);
	...
	if (!circ) {
		bcm63xx_console_write(co, s, count);
		return;
	}
	for (i = 0; i < count; i++) {
		if (uart_circ_chars_free(circ) > 1) {
			circ->buf[circ->head] = s[i];
		...
		else { /* no more room in our buffer */
			/* so we now have to transmit a char off the tail,
			   which is really the head of the queue but someones
			   preverse logic calls it the tail (say no to drugs).
			   At this stage we spin waiting till there is room
			   in the fifo. The presumption here is that something
			   has gone terribly wrong and we need to pump out the
			   information irrespective of interrupt latency issues.
			*/
			while ((UART_REG(port))->txf_levl >= port->fifosize);
			(UART_REG(port))->Data = circ->buf[circ->tail];
			circ->tail = (circ->tail + 1) & (UART_XMIT_SIZE - 1);
...
static struct circ_buf console_circ_buf;
...
static int __init bcm63xx_async_console_init(void)
{
	if (no_uart) return 0;
	printk("*** %s ***\n", __func__);
	console_circ_buf.buf = (unsigned char *)get_zeroed_page(GFP_KERNEL);
	if (!console_circ_buf.buf)
		return -ENOMEM;
	uart_circ_clear(&console_circ_buf);
	bcm63xx_ports[0].private_data = &console_circ_buf;
	bcm63xx_console.write = bcm63xx_async_console_write;
	return 0;
}
late_initcall(bcm63xx_async_console_init);
#endif
